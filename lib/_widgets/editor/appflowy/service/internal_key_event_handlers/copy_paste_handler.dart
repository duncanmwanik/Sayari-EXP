...
void _pasteMarkdown(EditorState editorState, String markdown) {
  ...
  transaction
    ...
    ..afterSelection = Selection.collapsed(Position(path: afterPath, offset: offset));
  editorState.apply(transaction);
}
...
void handlePastePlainText(EditorState editorState, String plainText) {
  ...
  final lines = plainText.split('\n').map((e) => e.replaceAll(RegExp(r'\r'), '')).toList();
...
void _pasteSingleLineInText(
  ...
  if (delta.isEmpty || insertedDelta == null) {
    ...
    transaction.afterSelection = Selection.collapsed(Position(path: selection.end.path, offset: length));
    editorState.apply(transaction);
  } else {
...
void _pasteMultipleLinesInText(
  ...
  final transaction = editorState.transaction;
  final selection = editorState.selection;
  final afterSelection = _computeSelectionAfterPasteMultipleNodes(editorState, nodes);
  ...
  if (selectionNode.length == 1) {
    ...
    if (nodes.length == 1 && nodes.first.type == node.type) {
      transaction.deleteNode(node);
      final List<dynamic> newDelta = firstNode.delta != null ? firstNode.delta!.toJson() : Delta().toJson();
      final List<Node> children = [];
      children.addAll(firstNode.children);

      if (nodes.first.delta != null && nodes.first.delta != null && nodes.first.delta!.isNotEmpty) {
        newDelta.addAll(nodes.first.delta!.toJson());
        children.addAll(nodes.first.children);
      }
      if (afterNode != null && afterNode.delta != null && afterNode.delta!.isNotEmpty) {
    ...
    transaction.insertNodes([
      ...
      firstNode,
      ...nodes,
      if (afterNode != null && afterNode.delta != null && afterNode.delta!.isNotEmpty) afterNode,
    ]);
    transaction.afterSelection = afterSelection;
    editorState.apply(transaction);
    return;
