...
StreamController<int> appFlowyEditorOnTapSelectionArea = StreamController<int>.broadcast();
...
class MobileSelectionServiceWidget extends StatefulWidget {
  ...
  @override
  State<MobileSelectionServiceWidget> createState() => _MobileSelectionServiceWidgetState();
}

class _MobileSelectionServiceWidgetState extends State<MobileSelectionServiceWidget>
  ...
  Widget _buildCollapsedHandle() {
    return ValueListenableBuilder(
      ...
      builder: (context, selection, _) {
        ...
        if (PlatformExtension.isAndroid && editorState.selectionUpdateReason != SelectionUpdateReason.uiEvent) {
          isCollapsedHandleVisible = false;
          return const SizedBox.shrink();
        }
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, selection, _) {
        ...
        final node = editorState.getNodeAtPath(
          handleType == HandleType.left ? selection.start.path : selection.end.path,
        );
        final selectable = node?.selectable;
        ...
        return MobileSelectionHandle(
          ...
          handleColor: isCollapsedWhenDraggingHandle ? Colors.transparent : editorStyle.dragHandleColor,
          handleWidth: editorStyle.mobileDragHandleWidth,
          handleBallWidth: editorStyle.mobileDragHandleBallSize.width,
          enableHapticFeedbackOnAndroid: editorStyle.enableHapticFeedbackOnAndroid,
        );
  ...
  void updateSelection(Selection? selection) {
    ...
    editorState.updateSelectionWithReason(
      ...
      extraInfo: {
        ...
        selectionExtraInfoDoNotAttachTextService: dragMode == MobileSelectionDragMode.cursor,
      },
    );
  }
  ...
  void _onLongPressUpdateAndroid(LongPressMoveUpdateDetails details) {
    ...
    final wordBoundary = getNodeInOffset(offset)?.selectable?.getWordBoundaryInOffset(offset);
    ...
    if (wordBoundary != null) {
      if (wordBoundary.end.path > _panStartSelection!.end.path ||
          wordBoundary.end.path.equals(_panStartSelection!.end.path) && wordBoundary.end.offset > _panStartSelection!.end.offset) {
        newSelection = Selection(
          start: _panStartSelection!.start,
          end: wordBoundary.end,
        ).normalized;
      } else if (wordBoundary.start.path < _panStartSelection!.start.path ||
          wordBoundary.start.path.equals(_panStartSelection!.start.path) && wordBoundary.start.offset < _panStartSelection!.start.offset) {
        newSelection = Selection(
  ...
  void _onPanUpdateAndroid(DragUpdateDetails details) {
    ...
    if (details.delta.dx.abs() < details.delta.dy.abs() && (_panStartOffset == null || _panStartScrollDy == null)) {
