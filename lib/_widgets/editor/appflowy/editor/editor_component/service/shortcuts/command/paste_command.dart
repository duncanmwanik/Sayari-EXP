...
final CommandShortcutEvent pasteTextWithoutFormattingCommand = CommandShortcutEvent(
...
CommandShortcutEventHandler _pasteTextWithoutFormattingCommandHandler = (editorState) {
...
extension on EditorState {
  Future<bool> pasteHtml(String html) async {
    ...
    while (nodes.isNotEmpty && nodes.first.delta?.isEmpty == true && nodes.first.children.isEmpty) {
      nodes.removeAt(0);
    }
    while (nodes.isNotEmpty && nodes.last.delta?.isEmpty == true && nodes.last.children.isEmpty) {
      nodes.removeLast();
    }
    if (nodes.isEmpty) {
      return false;
  ...
  Future<void> pastePlainText(String plainText) async {
    ...
    final nodes = plainText
        ...
        .map((paragraph) {
          Delta delta = Delta();
          if (_hrefRegex.hasMatch(paragraph) || _phoneRegex.hasMatch(paragraph)) {
            final match = _hrefRegex.firstMatch(paragraph) ?? _phoneRegex.firstMatch(paragraph);
            if (match != null) {
              ...
              if (entity != null) {
                ...
                delta.insert(
                  ...
                  attributes: {
                    AppFlowyRichTextKeys.href: _phoneRegex.hasMatch(entity) ? 'tel:$entity' : entity,
                  },
                );
