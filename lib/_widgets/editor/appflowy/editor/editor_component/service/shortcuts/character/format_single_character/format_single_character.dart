...
(bool, CheckSingleFormatFormatResult?) checkSingleCharacterFormatShouldBeApplied({
...
// If the selection is not collapsed or the cursor is at the first two index range, we don't need to format it.
  ...
  int startIndex = 0;
  if (lastInlineCode != null && formatStyle != FormatStyleByWrappingWithSingleChar.code) {
  ...
  if (rawPlainText.substring(startIndex + lastCharIndex + 1, selection.end.offset).isEmpty) {
    return (false, null);
  }
