...
class SearchServiceV3 {
  ...
  set selectedIndex(int index) {
    _selectedIndex = matchWrappers.value.isEmpty ? -1 : index.clamp(0, matchWrappers.value.length - 1);
  ...
  void navigateToMatch({bool moveUp = false}) {
    ...
    if (moveUp) {
      selectedIndex = selectedIndex <= 0 ? matchWrappers.value.length - 1 : selectedIndex - 1;
    } else {
      selectedIndex = selectedIndex >= matchWrappers.value.length - 1 ? 0 : selectedIndex + 1;
    }
  ...
  Future<void> replaceSelectedWord(String replaceText) async {
    if (replaceText.isEmpty || queriedPattern.isEmpty || matchWrappers.value.isEmpty) {
      return;
    }
