...
class SearchService {
  ...
  void findAndHighlight(String pattern, {bool unHighlight = false}) {
    ...
    for (final n in _getAllNodes()) {
      ...
      //is found.
      List<int> matches = searchAlgorithm.searchMethod(pattern, n.delta!.toPlainText()).map((e) => e.start).toList();
  ...
  void navigateToMatch({bool moveUp = false}) {
    ...
    if (moveUp) {
      selectedIndex = selectedIndex - 1 < 0 ? matchedPositions.length - 1 : --selectedIndex;
    } else {
      selectedIndex = (selectedIndex + 1) < matchedPositions.length ? ++selectedIndex : 0;
    }
  ...
  void replaceSelectedWord(String replaceText) {
    if (replaceText.isEmpty || queriedPattern.isEmpty || matchedPositions.isEmpty) {
      return;
    }
  ...
  void _applySelectedHighlightColor(
    ...
    final color = isSelected ? style.selectedHighlightColor.toHex() : style.unselectedHighlightColor.toHex();
    editorState.formatDelta(
      selection,
