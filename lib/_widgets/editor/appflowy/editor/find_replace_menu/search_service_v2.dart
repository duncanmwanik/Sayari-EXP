...
class SearchServiceV2 {
  ...
  set selectedIndex(int index) {
    ...
    _selectedIndex = matchedPositions.value.isEmpty ? -1 : index.clamp(0, matchedPositions.value.length - 1);
  ...
  void _highlightCurrentMatch(
    ...
    if (_prevSelectedIndex != selectedIndex &&
        ((_prevSelectedIndex == length && selectedIndex == 0) || (_prevSelectedIndex == 0 && selectedIndex == length))) {
  ...
  void navigateToMatch({bool moveUp = false}) {
    ...
    if (moveUp) {
      selectedIndex = selectedIndex <= 0 ? matchedPositions.value.length - 1 : selectedIndex - 1;
    } else {
      selectedIndex = selectedIndex >= matchedPositions.value.length - 1 ? 0 : selectedIndex + 1;
    }
  ...
  Future<void> replaceSelectedWord(String replaceText) async {
    if (replaceText.isEmpty || queriedPattern.isEmpty || matchedPositions.value.isEmpty) {
      return;
    }
