...
extension SelectionTransform on EditorState {
  ...
  Future<bool> deleteSelection(Selection selection) async {
    ...
    if (nodes.length == 1) {
      ...
      final node = nodes.first.type == TableCellBlockKeys.type ? nodes.first.children.first : nodes.first;
      if (node.delta != null) {
    ...
    // Otherwise, multiple nodes are selected, so we have to do more work.
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (i != 0) {
          ...
          else if (node.id == nodes.last.id && nodes.first.parent?.type == TableCellBlockKeys.type) {
            transaction.deleteText(
              node,
              0,
              selection.end.offset,
        ...
        if (nodes.last.delta != null && ![node.parent?.type, nodes.last.parent?.type].contains(TableCellBlockKeys.type)) {
        ...
        // Otherwise, we can just delete the selected text.
          ...
          if (nodes.last.parent?.type == TableCellBlockKeys.type || node.parent?.type == TableCellBlockKeys.type) {
            transaction.deleteText(
              node,
              selection.startIndex,
    ...
    AppFlowyEditorLog.editor.debug(transaction.operations.map((e) => e.toString()).toString());
  ...
  void moveCursor(
    ...
    final offset = direction == SelectionMoveDirection.forward ? selection.startIndex : selection.endIndex;
    {
      // the cursor is at the start of the node
      // move the cursor to the end of the previous node
      if (direction == SelectionMoveDirection.forward && start != null && start.offset >= offset) {
        final previousEnd = node.previousNodeWhere((element) => element.selectable != null)?.selectable?.end();
        if (previousEnd != null) {
      ...
      else if (direction == SelectionMoveDirection.backward && end != null && end.offset <= offset) {
    ...
    switch (range) {
          ...
          updateSelectionWithReason(
            Selection.collapsed(
              selection.start.copyWith(
                offset: direction == SelectionMoveDirection.forward ? delta.prevRunePosition(offset) : delta.nextRunePosition(offset),
              ),
            ),
            reason: SelectionUpdateReason.uiEvent,
          );
      ...
      case SelectionMoveRange.word:
        ...
        if (delta != null) {
          ...
          final wordSelection = node.selectable?.getWordBoundaryInPosition(position);
          if (wordSelection != null) {
            updateSelectionWithReason(
              Selection.collapsed(
                direction == SelectionMoveDirection.forward ? wordSelection.start : wordSelection.end,
              ),
              reason: SelectionUpdateReason.uiEvent,
            );
          }
      ...
      case SelectionMoveRange.line:
        if (delta != null) {
          ...
          updateSelectionWithReason(
            Selection.collapsed(
              selection.start.copyWith(
                offset: direction == SelectionMoveDirection.forward ? 0 : delta.length,
              ),
            ),
            reason: SelectionUpdateReason.uiEvent,
          );
