...
mixin DefaultSelectableMixin {
  ...
  Position getPositionInOffset(Offset start) => forward.getPositionInOffset(start);
  ...
  Selection getSelectionInRange(Offset start, Offset end) => forward.getSelectionInRange(start, end);
  ...
  Selection? getWordEdgeInOffset(Offset offset) => forward.getWordEdgeInOffset(offset);

  Selection? getWordBoundaryInOffset(Offset offset) => forward.getWordBoundaryInOffset(offset);

  Selection? getWordBoundaryInPosition(Position position) => forward.getWordBoundaryInPosition(position);

  Position start() => forward.start();

  Position end() => forward.end();

  TextDirection textDirection() => forwardKey.currentState != null ? forward.textDirection() : TextDirection.ltr;
}
