...
CommandShortcutEventHandler _enterInTableCellHandler = (editorState) {
  ...
  if (_hasSelectionAndTableCell(inTableNodes, selection)) {
    ...
    if (nextNode == null) {
      ...
      transaction.afterSelection = Selection.single(path: cell.parent!.path.next, startOffset: 0);
...
CommandShortcutEventHandler _rightInTableCellHandler = (editorState) {
  final inTableNodes = _inTableNodes(editorState);
  final selection = editorState.selection;
  if (_hasSelectionAndTableCell(inTableNodes, selection) && selection!.start.offset == inTableNodes.first.delta!.length) {
...
CommandShortcutEventHandler _upInTableCellHandler = (editorState) {
  ...
  if (_hasSelectionAndTableCell(inTableNodes, selection)) {
    ...
    if (_nodeHasTextChild(nextNode)) {
      final target = nextNode!.childAtIndexOrNull(0)!;
      final off = target.delta!.length > selection!.start.offset ? selection.start.offset : target.delta!.length;
      editorState.selectionService.updateSelection(
...
bool _hasSelectionAndTableCell(
  ...
  Selection? selection,
    ...
    nodes.length == 1 && selection != null && selection.isCollapsed && nodes.first.parent?.type == TableCellBlockKeys.type;
...
Node? _getNextNode(Iterable<Node> nodes, int colDiff, int rowDiff) {
  ...
  final numCols = table.children.last.attributes[TableCellBlockKeys.colPosition] + 1;
  final numRows = table.children.last.attributes[TableCellBlockKeys.rowPosition] + 1;
  ...
  return isValidPosition(nextCol, nextRow, numCols, numRows) ? getCellNode(table, nextCol, nextRow) : null;
}
...
Node? _getPreviousNode(Iterable<Node> nodes, int colDiff, int rowDiff) {
  ...
  return isValidPosition(prevCol, prevRow, numCols, numRows) ? getCellNode(table, prevCol, prevRow) : null;
}

bool isValidPosition(int col, int row, int numCols, int numRows) => col >= 0 && col < numCols && row >= 0 && row < numRows;

bool _nodeHasTextChild(Node? n) => n != null && n.children.isNotEmpty && n.childAtIndexOrNull(0)!.delta != null;
