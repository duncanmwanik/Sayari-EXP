...
mixin BlockComponentTextDirectionMixin {
  ...
  TextDirection calculateTextDirection({TextDirection? layoutDirection}) {
    ...
    if (node.level > 1 && direction != lastDirection && node.direction(defaultTextDirection) == blockComponentTextDirectionAuto) {
      WidgetsBinding.instance.addPostFrameCallback((_) => node.parent?.notify());
    }
...
TextDirection calculateNodeDirection({
  ...
  if (value == blockComponentTextDirectionAuto) {
    if (lastDirection != null) {
      ...
      defaultTextDirection = _getDirectionFromPreviousOrParentNode(node, defaultTextDirection)?.name ?? defaultTextDirection;
    }
  }
  ...
  return determineTextDirection(text) ?? defaultTextDirection?.toTextDirection() ?? layoutDirection;
}
...
TextDirection? _getDirectionFromNode(Node node, String? defaultTextDirection) {
  final nodeDirection = node.direction(
    defaultTextDirection == blockComponentTextDirectionAuto ? blockComponentTextDirectionAuto : null,
  );
...
extension on Node {
  String? direction(String? defaultDirection) => attributes[blockComponentTextDirection] as String? ?? defaultDirection;
}
