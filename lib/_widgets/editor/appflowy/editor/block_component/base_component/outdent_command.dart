...
bool isOutdentable(EditorState editorState) {
  ...
  nodes = nodes.where((node) => node.path.length == parent.path.length + 1).toList();

  final isAllIndentable = nodes.every((node) => indentableBlockTypes.contains(node.type));
  if (!isAllIndentable) {
    return false;
  }
...
CommandShortcutEventHandler _outdentCommandHandler = (editorState) {
  ...
  final startPath = nodes.first.path.sublist(0, nodes.first.path.length - 1)..last += 1;
  final endPath = nodes.last.path.sublist(0, nodes.last.path.length - 1)..last += nodes.length;
