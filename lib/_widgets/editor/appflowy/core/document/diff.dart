...
List<Operation> diffNodes(Node oldNode, Node newNode) {
  ...
  // Identify deletions
  oldChildrenById.keys.where((id) => !newChildrenById.containsKey(id)).forEach((id) {
  ...
  if (operations.isNotEmpty && operations.every((element) => element is InsertOperation)) {
  ...
  if (operations.isNotEmpty && operations.every((element) => element is DeleteOperation)) {
    operations.sorted((a, b) => a.path <= b.path ? -1 : 1);
    for (var i = 0; i < operations.length; i++) {
      final op = operations[i];
      if (combinedOperations.isEmpty) {
