...
class HtmlTableNodeParser extends HTMLNodeParser {
  ...
  List<dom.Node> transformNodeToDomNodes(
    ...
    final int rowsLen = node.attributes[TableBlockKeys.rowsLen], colsLen = node.attributes[TableBlockKeys.colsLen];
    ...
    for (var i = 0; i < rowsLen; i++) {
      ...
      final rowelement = wrapChildrenNodesWithTagName(HTMLTags.tableRow, childNodes: nodes);

      domNodes.add(rowelement);
    }

    final element = wrapChildrenNodesWithTagName(HTMLTags.table, childNodes: domNodes);
    return [
      element,
    ];
  }
