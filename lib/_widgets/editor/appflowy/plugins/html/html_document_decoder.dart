...
class DocumentHTMLDecoder extends Converter<String, Document> {
  ...
  Document convert(String input) {
    ...
    final parseForSingleChild = body.children.length == 1 && HTMLTags.formattingElements.contains(body.children.first.localName);
    return Document.blank(withInitialText: false)
      ..insert(
        [0],
        parseForSingleChild ? _parseElement(body.children.first.children) : _parseElement(body.nodes),
      );
  }
  ...
  Node _parseListElement(
    ...
    if (delta.isEmpty && element.children.length == 1 && element.children.first.localName == HTMLTags.paragraph) {
...
class HTMLTags {
  ...
  static bool isTopLevel(String tag) {
    return tag == h1 || tag == h2 || tag == h3 || tag == table || tag == checkbox || tag == paragraph || tag == div || tag == blockQuote;
  }
}
