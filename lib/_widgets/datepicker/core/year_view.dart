    ...
    if (widget.cellBuilder != null && _children.isEmpty) {
      ...
      for (int j = 0; j < viewCount; j++) {
        ...
        final double viewStartPosition =
            isVerticalMultiView ? 0 : (currentViewIndex * width) + (currentViewIndex * widget.multiViewSpacing);
        ...
        for (int i = 0; i < visibleDatesCount; i++) {
          ...
          if (widget.isRtl) {
            ...
            currentIndex = DateRangePickerHelper.getRtlIndex(YearView.maxColumnCount, i % YearView.maxColumnCount) +
...
abstract class _IYearViewRenderObject extends RenderBox with ContainerRenderObjectMixin<RenderBox, _DatePickerParentData> {
  ...
  void drawSelection(Canvas canvas, double cellWidth, int currentIndex, double highlightPadding, double selectionPadding,
      double textHalfHeight, double centerYPosition, double xPosition, double yPosition, TextSpan yearText);
  ...
  List<int> _getRangeIndex(dynamic startDate, dynamic endDate, DateRangePickerView pickerView, int viewStartIndex, int viewEndIndex) {
    ...
    if (startDate != null) {
      ...
      if (viewStartDate.isAfter(startDate) == true && viewStartDate.isBefore(endRangeDate) == true) {
        ...
        startIndex = DateRangePickerHelper.getDateCellIndex(visibleDates, startDate, pickerView,
    ...
    if (endRangeDate != null) {
      ...
      if (viewEndDate.isAfter(startDate) == true && viewEndDate.isBefore(endRangeDate) == true) {
        ...
        endIndex = DateRangePickerHelper.getDateCellIndex(visibleDates, endRangeDate, _view,
            viewStartIndex: viewStartIndex, viewEndIndex: viewEndIndex);
      }
    }
  ...
  void _addMouseHovering(
    ...
    if (xPosition <= _mouseHoverPosition.value!.offset!.dx &&
      ...
      _todayHighlightPaint.color =
          selectionColor != null ? selectionColor!.withOpacity(0.4) : datePickerTheme.selectionColor!.withOpacity(0.4);
  ...
  void _addRangeHoverEffect(
    ...
    final double cornerRadius =
        isBetweenRange ? 0 : (selectionShape == DateRangePickerSelectionShape.circle ? (endYPosition - startYPosition) / 2 : 3);
  ...
  void _drawTodayHighlight(Canvas canvas, double cellWidth, double cellHeight, double centerYPosition, double highlightPadding,
      double selectionPadding, double textHalfHeight, double xPosition, double yPosition) {
  ...
  void _drawYearDecoration(Canvas canvas, Decoration yearDecoration, double xPosition, double yPosition, double decorationPadding,
...
class _SingleSelectionRenderObject extends _IYearViewRenderObject {
  ...
  List<int> getSelectedIndex(int viewStartIndex, int viewEndIndex) {
    ...
    final int index = DateRangePickerHelper.getDateCellIndex(visibleDates, _selectedDate, _view,
        viewStartIndex: viewStartIndex, viewEndIndex: viewEndIndex);
    if (index != -1) {
...
class _MultipleSelectionRenderObject extends _IYearViewRenderObject {
  ...
  List<int> getSelectedIndex(int viewStartIndex, int viewEndIndex) {
    ...
    for (int i = 0; i < _selectedDates!.length; i++) {
      final int index = DateRangePickerHelper.getDateCellIndex(visibleDates, _selectedDates![i], _view,
          viewStartIndex: viewStartIndex, viewEndIndex: viewEndIndex);
      if (index != -1) {
        selectedIndex.add(index);
      }
...
void _drawYearCells(PaintingContext context, Size size, _IYearViewRenderObject yearView) {
  ...
  double height = size.height;
  final bool isHorizontalMultiView =
      yearView.enableMultiView && yearView.navigationDirection == DateRangePickerNavigationDirection.horizontal;
    ...
    for (int j = 0; j < count; j++) {
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        final bool isEnableDate = DateRangePickerHelper.isBetweenMinMaxDateCell(
            date, yearView.minDate, yearView.maxDate, yearView.enablePastDates, view, yearView.isHijri);
        ...
        if (!isDisabledDate &&
            ...
            DateRangePickerHelper.isDisableDirectionDate(
                yearView.selectedRange, date, yearView.extendableRangeSelectionDirection, view, yearView.isHijri,
        ...
        if (isSelected && isEnableDate && !isDisabledDate) {
          yearView.drawCustomCellSelection(
              canvas, Rect.fromLTRB(xPosition, yPosition, xPosition + cellWidth, yPosition + cellHeight), currentIndex);
        }
  ...
  yearView._textPainter.textScaler = TextScaler.linear(yearView.textScaleFactor);
  ...
  for (int j = 0; j < count; j++) {
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      if (yearView.isRtl) {
        final int rowIndex = i ~/ YearView.maxColumnCount;
        currentIndex =
            DateRangePickerHelper.getRtlIndex(YearView.maxColumnCount, i % YearView.maxColumnCount) + (rowIndex * YearView.maxColumnCount);
      }
      ...
      if (isSelected && isEnableDate && !isDisabledDate) {
        yearView.drawSelection(canvas, cellWidth, currentIndex, highlightPadding, selectionPadding, textHalfHeight, centerYPosition,
            xPosition, yPosition, yearText);
        ...
        yearView._drawTodayHighlight(
            canvas, cellWidth, cellHeight, centerYPosition, highlightPadding, selectionPadding, textHalfHeight, xPosition, yPosition);
      }
      ...
      if (hoveringIndex != null && hoveringIndex.isNotEmpty && hoveringIndex.contains(currentIndex) && isEnableDate) {
        yearView._addRangeHoverEffect(canvas, xPosition, yPosition, currentIndex, hoveringIndex, yearView, centerYPosition, textHalfHeight,
            selectionPadding, highlightPadding, cellWidth, cellHeight);
      }
      ...
      if ((!isSelected || (yearView.isBetweenRange(currentIndex))) &&
          ...
          yearView.mouseHoverPosition.value != null &&
          yearView.mouseHoverPosition.value!.offset != null) {
        yearView._addMouseHovering(canvas, cellWidth, cellHeight, centerYPosition, currentViewIndex, width, highlightPadding, date,
            selectionPadding, textHalfHeight, webUIPadding, xOffset, xPosition, yOffset, yPosition);
      }
