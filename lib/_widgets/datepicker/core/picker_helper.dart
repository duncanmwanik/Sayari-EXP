...
class DateRangePickerHelper {
  ...
  static bool isDisableDirectionDate(dynamic range, dynamic currentDate,
      ExtendableRangeSelectionDirection extendableRangeSelectionDirection, DateRangePickerView view, bool isHijri,
  ...
  static bool canMoveToPreviousViewRtl(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate, dynamic maxDate,
      List<dynamic> visibleDates, bool isRtl, bool enableMultiView, bool isHijri) {
    if (isRtl) {
  ...
  static bool canMoveToNextViewRtl(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate, dynamic maxDate,
      List<dynamic> visibleDates, bool isRtl, bool enableMultiView, bool isHijri) {
    if (isRtl) {
      return canMoveToPreviousView(view, numberOfWeeksInView, minDate, visibleDates, enableMultiView, isHijri);
    } else {
      ...
      case DateRangePickerView.month:
        {
          if (numberOfWeeksInView != 6 && !isHijri) {
            ...
            if ((previousDate.month < minDate.month == true && previousDate.year == minDate.year) ||
  ...
  static bool isBetweenMinMaxDateCell(dynamic date, dynamic minDate, dynamic maxDate, bool enablePastDates, dynamic view, bool isHijri) {
    ...
    if (pickerView == DateRangePickerView.year) {
      return ((date.month >= minDate.month == true && date.year == minDate.year) || date.year > minDate.year == true) &&
          ...
          (enablePastDates ||
              (!enablePastDates && ((date.month >= today.month == true && date.year == today.year) || date.year > today.year == true)));
    } else if (pickerView == DateRangePickerView.decade) {
  ...
  static getHijriPickerView(DateRangePickerView century) {}
}
