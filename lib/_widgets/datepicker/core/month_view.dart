...
class _MonthViewState extends State<MonthView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.cellBuilder != null && _children.isEmpty) {
      ...
      int viewCount = 1;
      final bool isHorizontalMultiView =
          widget.enableMultiView && widget.navigationDirection == DateRangePickerNavigationDirection.horizontal;
      ...
      for (int j = 0; j < viewCount; j++) {
        ...
        if (widget.isRtl) {
          viewStartPosition = isVerticalMultiView
              ? 0
              : (currentViewIndex * width) + (currentViewIndex * webUIPadding) + (weekNumberPanelWidth * currentViewIndex);
        }
        ...
        for (int i = 0; i < datesCount; i++) {
          ...
          if (widget.isRtl) {
            ...
            currentIndex =
                DateRangePickerHelper.getRtlIndex(DateTime.daysPerWeek, i % DateTime.daysPerWeek) + (rowIndex * DateTime.daysPerWeek);
          }
          ...
          final Widget child = widget.cellBuilder(
              ...
              widget.isHijri
                  ? HijriDateRangePickerCellDetails(
                      ...
                      bounds: Rect.fromLTWH(xPosition, yPosition, cellWidth, cellHeight))
...
void _drawSelectedDate(Canvas canvas, double radius, double centerXPosition, double cellWidth, double cellHeight, double x, double y,
    _IMonthView view, double centerYPosition) {
...
void _drawStartAndEndRange(Canvas canvas, _IMonthView view, double cellHeight, double cellWidth, double radius, double centerXPosition,
    double centerYPosition, double x, double y, Color color, double heightDifference, bool isStartRange) {
  switch (view.selectionShape) {
    case DateRangePickerSelectionShape.circle:
      {
        Rect rect;
...
void _drawFillSelection(Canvas canvas, double x, double y, double width, double height, Paint selectionPainter) {
  ...
  canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTRB(x + padding, y + padding, x + width - padding, y + height - padding),
...
void _drawMonthCellsAndSelection(PaintingContext context, Size size, _IMonthView monthView, double cellWidth, double cellHeight) {
  ...
  int viewCount = 1;
  final bool isHorizontalMultiView =
      monthView.enableMultiView && monthView.navigationDirection == DateRangePickerNavigationDirection.horizontal;
  final bool isVerticalMultiView =
      monthView.enableMultiView && monthView.navigationDirection == DateRangePickerNavigationDirection.vertical;
  ...
  monthView._textPainter.textScaler = TextScaler.linear(monthView.textScaleFactor);
      ...
      for (int i = 0; i < datesCount; i++) {
        ...
        if (weekNumberPanelWidth != 0 &&
            ...
            ((i <= DateTime.daysPerWeek && monthView.visibleDates[viewStartIndex + DateTime.daysPerWeek].month == currentMonth) ||
                ...
                (i >= datesCount - DateTime.daysPerWeek &&
                    (monthView.visibleDates[viewEndIndex - (DateTime.daysPerWeek - 1)].month == currentMonth))) &&
        ...
        final bool isEnableDate =
            DateRangePickerHelper.isEnabledDate(monthView.minDate, monthView.maxDate, monthView.enablePastDates, date, monthView.isHijri);
        ...
        bool isDisabledDate =
            DateRangePickerHelper.isDateWithInVisibleDates(monthView.visibleDates, monthView.disableDatesCollection, date);
        if (!isDisabledDate &&
        ...
        if (monthView.mouseHoverPosition.value != null && monthView.mouseHoverPosition.value!.offset != null) {
          ...
          if (xPosition <= monthView.mouseHoverPosition.value!.offset!.dx &&
            ...
            canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(xPosition, yPosition, cellWidth, cellHeight), const Radius.circular(2)),
  ...
  for (int j = 0; j < viewCount; j++) {
    ...
    for (int i = 0; i < datesCount; i++) {
      ...
      textStyle = _updateTextStyle(
          monthView, isNextMonth, isPreviousMonth, isCurrentDate, isEnableDate, isBlackedDate, isWeekEnd, isSpecialDate, isDisabledDate);
      dateDecoration = _updateDecoration(isNextMonth, isPreviousMonth, monthView, isEnableDate, isCurrentDate, isBlackedDate, date,
          isWeekEnd, isSpecialDate, isDisabledDate);
      ...
      if (isSelectedDate &&
        ...
        textStyle =
            _drawCellAndSelection(canvas, xPosition, yPosition, selectionTextStyle, selectedRangeTextStyle, monthView, currentIndex);
...
void _drawWeekNumber(Canvas canvas, Size size, dynamic date, double cellHeight, double yPosition, double weekNumberPanelWidth,
    _IMonthView monthView, double viewStartPosition, double viewEndPosition) {
...
void _addRangeHoverEffect(
    Canvas canvas, double xPosition, double yPosition, _IMonthView monthView, int currentIndex, List<int>? hoveringIndex) {
  ...
  final double radius = _getCellRadius(
      rangeSelectionMonthView.selectionRadius, rangeSelectionMonthView._centerXPosition, rangeSelectionMonthView._centerYPosition);
  ...
  if (isStartRange) {
    switch (monthView.selectionShape) {
      case DateRangePickerSelectionShape.circle:
        rect = Rect.fromLTRB(xPosition + rangeSelectionMonthView._centerXPosition, yPosition + heightDifference,
            xPosition + rangeSelectionMonthView._cellWidth, yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
        break;
      case DateRangePickerSelectionShape.rectangle:
        final double rightPosition = xPosition + rangeSelectionMonthView._cellWidth;
        rect = Rect.fromLTRB(rightPosition - cornerRadius, yPosition + heightDifference, rightPosition,
    ...
    switch (monthView.selectionShape) {
      ...
      case DateRangePickerSelectionShape.rectangle:
        {
          rect = Rect.fromLTRB(xPosition, yPosition + heightDifference, xPosition + cornerRadius,
              yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
        }
    }
  } else if (isBetweenRange) {
...
void _addHoveringEffect(
    Canvas canvas, _IMonthView monthView, double xPosition, double yPosition, double cellWidth, double cellHeight, Size size) {
  if (xPosition <= monthView.mouseHoverPosition.value!.offset!.dx &&
      xPosition + cellWidth >= monthView.mouseHoverPosition.value!.offset!.dx &&
      yPosition <= monthView.mouseHoverPosition.value!.offset!.dy &&
      yPosition + cellHeight >= monthView.mouseHoverPosition.value!.offset!.dy) {
    ...
    switch (monthView.selectionShape) {
      ...
      case DateRangePickerSelectionShape.rectangle:
        {
          canvas.drawRRect(
              RRect.fromRectAndRadius(Rect.fromLTWH(xPosition + 1, yPosition + 1, cellWidth - 1, cellHeight - 1),
...
TextStyle _drawCellAndSelection(Canvas canvas, double xPosition, double yPosition, TextStyle selectionTextStyle,
    TextStyle selectedRangeTextStyle, _IMonthView monthView, int currentIndex) {
...
void _drawDecoration(Canvas canvas, double xPosition, double yPosition, double padding, double cellWidth, double cellHeight,
    Decoration dateDecoration, _IMonthView monthView) {
  final BoxPainter boxPainter = dateDecoration.createBoxPainter(monthView.markNeedsPaint);
  boxPainter.paint(canvas, Offset(xPosition + padding, yPosition + padding),
      ImageConfiguration(size: Size(cellWidth - (2 * padding), cellHeight - (2 * padding))));
}

void _drawCurrentDate(
    Canvas canvas, _IMonthView monthView, double xPosition, double yPosition, double padding, double cellWidth, double cellHeight) {
    ...
    case DateRangePickerSelectionShape.rectangle:
      {
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromLTRB(xPosition + padding, yPosition + padding, xPosition + cellWidth - padding, yPosition + cellHeight - padding),
                Radius.circular(cellHeight / 4 > 10 ? 10 : cellHeight / 4)),
            monthView._selectionPainter);
      }
  }
...
TextStyle _updateTextStyle(_IMonthView monthView, bool isNextMonth, bool isPreviousMonth, bool isCurrentDate, bool isEnableDate,
    bool isBlackedDate, bool isWeekEnd, bool isSpecialDate, bool isDisabledDate) {
...
Decoration? _updateDecoration(bool isNextMonth, bool isPreviousMonth, _IMonthView monthView, bool isEnableDate, bool isCurrentDate,
    bool isBlackedDate, dynamic date, bool isWeekEnd, bool isSpecialDate, bool isDisabledDate) {
